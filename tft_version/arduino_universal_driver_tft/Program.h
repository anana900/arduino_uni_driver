    ...
    for(int i = 0 ; i < CONDITION_SIZE ; i++)
    {
      if(active == condition_list[i].get_condition_status())
      {
        ...
        if(0 != condition_list[i].get_timeout() && none == condition_list[i].get_p1() && none == condition_list[i].get_p2())
        {
          ...
          Program::update_wy(condition_list[i].get_wy1(), condition_list[i].get_wy2());
          Serial.println((unsigned long)condition_list[i].get_timeout()*1000);
          Serial.println(condition_list[i].timer_actual);
          while(true == condition_list[i].timer_lock && millis() - (unsigned long)condition_list[i].timer_actual <= (unsigned long)(condition_list[i].get_timeout()*1000)){
            if(stop_program(millis()- working_time_start)) break;
                      Serial.println((unsigned long)condition_list[i].get_timeout()*1000);
          Serial.println(condition_list[i].timer_actual);
          Serial.println(millis() - (unsigned long)condition_list[i].timer_actual);
          }
